...
const TrendingTv = () => {
  ...
  const { data, isLoading, isError, isSuccess, error } = useQuery({
    ...
    queryFn: () => tmdb.trending.trending("tv", "week"),
    staleTime: Infinity,
  })

  const bigGenre = useGetGenreById(data?.results[0].genre_ids, "tv")
      ...
      <section>
        <div className="relative mb-12 w-full">
          <div className="relative aspect-video w-full bg-muted">
            ...
            <div className="absolute left-0 top-0 h-full w-full bg-gradient-to-b from-transparent to-background/80">
              ...
              <div className="absolute right-0 top-0 bg-background/50 lg:p-2">
                <div className="flex flex-col items-center justify-center text-sm lg:h-12 lg:w-12">
          ...
          <div className="mt-1 flex gap-1">
            ...
            <Button
              ...
              asChild
            >
              <Link to={`/tv/id/${data.results[0].id}`}>
        ...
        <MediaCarousel>
          {data.results.map((m, i) => {
            if (i == 0) return null
            else return <MediaItem key={i} data={m} mediaType="tv" />
          })}
        </MediaCarousel>
      </section>
    )
